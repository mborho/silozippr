...
var _BASE_CLASSES = "grid-10 round-5";
...
function renderItem(doc, combined) {    
    ...
    html += '<a class="line-del" onclick="Controller.deleteDoc(\'box-'+doc._id+'\');" >&#160;</a>';
    html += '</div>';
...
function renderList(docs, combined, limit, append, res) {
    ...
    for(var x = 0;max_for > x;x++) {    
        ...
        if(docs.rows[x].value.type== "tweet") {
            html += renderTweet(docs.rows[x].value);
        } else {
            html += renderItem(docs.rows[x].value, combined);
        }
    }
    if(max == max_for+1) {
...
function renderJsonItem(doc, combined) {    
    var item = { 
            kind: "NewsItem",  
            source:doc.skey,
            rev: doc._rev,
...
function renderJsonList(docs, combined, limit, append, res) {
    var html = ''
        , max = docs.rows.length
        , max_for = (limit && max > limit) ? max-1 : max
        , items = []
    ...
    for(var x = 0;max_for > x;x++) {    
        ...
        items[x] = {skey: docs.rows[x].value.skey, feed: { title: docs.rows[x].value.feed.title }};        
        if(docs.rows[x].value.type== "tweet") {
            items[x] = renderJsonTweet(docs.rows[x].value);
        } else {
            items[x] = renderJsonItem(docs.rows[x].value, combined);
        }
    }
//     if(max == max_for+1) {
//         html += renderNext(docs[max_for].value);
