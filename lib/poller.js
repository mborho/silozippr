...
var Poller = function(db) {
    ...
    function _fetch_feed(sub) {
        var feed = new FeedLoader(sub.url, function(err, feed) {
            if(!err) {
                try {
                    ...
                    sub.links = sub.links.slice(-200);
                    _db.merge(sub._id, sub, function(couch_err, couch_res) {
                        if(couch_err) console.log(couch_err);                  
                    });
                } catch(e) {
                    console.log(e);     
                }
