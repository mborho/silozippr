...
var Poller = function(db, conf) {
    ...
    function _process_string(pushToken, xml) {
        console.log('check for pushtoken: '+pushToken);
            ...
            if(!err) {
                if(docs.length == 1) {
                    ...
                    var feedPushed = new FeedPushed(sub, xml, function(feedErr, feed) {
                        _process_feed(feedErr, sub, feed);
                    });
                    feedPushed.process();
                }/* else {
                    console.log("sub not known, unsubscribe!");
                    var feedString = new FeedString(xml, function(feedErr, feed) {
                        console.log("notification unknown");
                        console.log(feed);
                        if(!feedErr) {
                            var p = new PubSubHubBub(_db,_conf).unsubscribe(pushToken, feed, function(pushErr, unsubscribed) {
                                if(pushErr) console.log(pushErr);
                            });
                        } else console.log(feedErr);
                    });
                    feedString.parse();                    
                }*/
            } else {
    ...
    return {
        ...
        fromString: function(pushToken, xml) {
            _process_string(pushToken, xml);
        }
    }    
}    
