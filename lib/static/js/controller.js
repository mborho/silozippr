...
var Controller = (function(socket, toc) {
    ...
    return {          
        ...
        translate: function(box_id) {
            try {
                var text = $('#'+box_id+' .line-body .twext').text(),
                    tlateUrl = 'http://translate.google.de/#auto|de|'+encodeURIComponent(text);
                window.open(tlateUrl, '', '');
            } catch(err) {
                console.log(err);
            }
            return false;
        ...
        translateUrl: function(url) {
            tlateUrl = 'http://translate.google.de/translate?sl=auto&tl=de&u='+encodeURIComponent(url);
            window.open(tlateUrl, '', '');
            return false;
        },        
