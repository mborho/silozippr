...
Mainline.prototype.addDoc = function(doc, pushed) {    
    ...
    if(!this.expanded && box.hasClass('news')) {
        box.find('.line-body').hide();
    }
    this.count++;
...
Mainline.prototype.append = function(data) {
    ...
    if(!this.expanded) {
        newItems.filter('.news').find('.line-body').hide();
    }
...
Mainline.prototype.toggleExpand = function() {
    if(this.expanded) {
        this.container.find('.news .line-body').fadeOut();                                   
        this.expanded = false;
    } else {          
        this.container.find('.news .line-body').show(); 
        this.expanded = true;
    }   
}
