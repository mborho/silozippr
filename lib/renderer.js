...
var Renderer =  function(withTwitter) {
    ...
    function _renderJsonItem(doc, combined) {
        var item = {
                ...
                href: doc.link,
                body: ((doc.summary !== false && doc.summary !== undefined) ? sani(doc.summary, true, true) : ''),
            }
        return item;
    }
    ...
    return  {
        renderPushed: function(doc) {
            ...
            if(doc.type == "tweet") {
                newDoc = _renderJsonTweet(doc);
                newDoc.html = _renderTweet(doc);
            } else {
                newDoc = _renderJsonItem(doc);
                newDoc.html = _renderItem(doc, true);
            }
            return newDoc;
        },
