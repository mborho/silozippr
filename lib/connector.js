...
Connector.prototype.get_index_json = function(res, startkey) {
    var limit = this.list_limit,
        start = (startkey) ? JSON.parse(startkey) : false,
        query = {
            descending: true,
    ...
    this.db.view('sources/all', query, function(err, docs) {  
        if (err) throw new Error(JSON.stringify(err));        
        renderer.json(docs, true, limit, (startkey !== false), res);                     
    });     
}

Connector.prototype.get_source_json = function(res, skey, startkey) { 
    var limit = this.list_limit,
        start = (startkey) ? JSON.parse(startkey) : false,
        query = {            
            descending: true,
    ...
    this.db.view('sources/by_feed_link', query, function(err, docs) {
        if (err) throw new Error(JSON.stringify(err));
        renderer.json(docs, false, limit, (startkey !== false), res);                                      
    });
}
