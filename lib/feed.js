...
Feed.prototype.setLinks = function(data) {
    ...
    if(links.length > 0) {
        for(var x=0; links.length > x; x++) {
            if(links[x].rel !== undefined) {
                if(links[x].rel === 'hub') pubsub = links[x].href;
                else if(links[x].rel === 'alternate' && (links[x].type == undefined || links[x].type == 'text/html'))  link = links[x].href; 
            }
        }        
    }
